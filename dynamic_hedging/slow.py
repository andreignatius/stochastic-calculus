...
for steps in hedging_steps:
    ...
    for i in range(paths):
        ...
        for t, S_t in zip(T, blackscholespath):
            ...
            if t == 0.0:
                ...
                hedging_errors.append( (prev_phi*S_t - stock_pos) + (prev_bond_pos*np.exp(r*dt) - bond_pos) )
    ...
    plt.bar(edges[:-1], percentages, width=np.diff(edges), align="edge", alpha=0.5, label=f'N={steps}')
