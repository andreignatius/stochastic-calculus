...
def implied_volatility(S, K, r, price, T, payoff):
    try:
        if payoff.lower() == "call":
            # print('call impl')
            impliedVol = brentq(
                lambda x: price - black_scholes_call(S, K, r, x, T), 1e-12, 10.0
            )
        elif payoff.lower() == "put":
            # print('put impl')
            impliedVol = brentq(
                lambda x: price - black_scholes_put(S, K, r, x, T), 1e-12, 10.0
            )
        else:
...
for i, days in enumerate(days_to_expiry):
    ...
    for strike, vol in adjusted_summary_list:
        if strike > S:
            dd_price_call = displaced_diffusion_call(
                F, strike, rate, sigma, T, trial_beta
            )
            dd_price.append(dd_price_call)
            dd_impl_vol.append(
                implied_volatility(S, strike, rate, dd_price_call, T, "call")
            )
        else:
            dd_price_put = displaced_diffusion_put(
                F, strike, rate, sigma, T, trial_beta
            )
            dd_price.append(dd_price_put)
            dd_impl_vol.append(
                implied_volatility(S, strike, rate, dd_price_put, T, "put")
            )
...
for i, days in enumerate(days_to_expiry):
    ...
    result = least_squares(
        dd_calibration,
        initial,
        args=(S, strikes, rate, sigma, T, market_vols),
        bounds=(0.5, 1),
    )
    beta = result.x[0]
...
ax1.plot(
    summary_df["strike"],
    summary_df["impliedvol17"],
    ".",
    label=f"{filename}_20201218 expiry",
)
ax1.plot(
    summary_df["strike"],
    dd_summary_df["dd_impliedvol17"],
    ".",
    color="red",
    label="displaced_diff_20201218 expiry",
)
...
ax1.legend()
ax1.set_title(
    f"{filename} : Implied Volatilities for 20201218 expiry; sigma = {sigma_beta_pair[0][0]:.5f} ; beta = {sigma_beta_pair[0][1]}"
)
...
ax2.plot(
    summary_df["strike"],
    summary_df["impliedvol45"],
    "+",
    label=f"{filename}_20210115 expiry",
)
ax2.plot(
    summary_df["strike"],
    dd_summary_df["dd_impliedvol45"],
    "+",
    color="red",
    label="displaced_diff_20210115 expiry",
)
...
ax2.legend()
ax2.set_title(
    f"{filename} : Implied Volatilities for 20210115 expiry ; sigma = {sigma_beta_pair[1][0]:.5f} ; beta = {sigma_beta_pair[1][1]}"
)
...
ax3.plot(
    summary_df["strike"],
    summary_df["impliedvol80"],
    "x",
    label=f"{filename}_20210219 expiry",
)
ax3.plot(
    summary_df["strike"],
    dd_summary_df["dd_impliedvol80"],
    "x",
    color="red",
    label="displaced_diff_20210219 expiry",
)
...
ax3.legend()
ax3.set_title(
    f"{filename} : Implied Volatilities for 20210219 expiry ; sigma = {sigma_beta_pair[2][0]:.5f} ; beta = {sigma_beta_pair[2][1]}"
)
