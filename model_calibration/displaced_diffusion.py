...
def impliedVolatility_displaced(S, K, r, price, T, payoff, beta):
    try:
        if (payoff.lower() == 'call'):
            impliedVol = brentq(lambda x: price - BlackScholesLognormalCall_displaced(S, K, r, x, T, beta), 1e-12, 10.0)
        elif (payoff.lower() == 'put'):
            impliedVol = brentq(lambda x: price - BlackScholesLognormalPut_displaced(S, K, r, x, T, beta), 1e-12, 10.0)
        else:
...
df['vols'] = df.apply(lambda x: impliedVolatility_displaced(S,
                                                  x['strike'],
                                                  r,
                                                  x['mid'],
                                                  T,
                                                  x['payoff'],
                                                  beta_constant),  # passing beta here
                      axis=1)
...
res_displaced = least_squares(lambda x: displacement_calibration(x, df['strike'], df['impliedvol'], S, r, T), initialGuess_displaced)
...
displaced_prices = [BlackScholesLognormalCall_displaced(S, K, r, sigma, T, beta) if K > S else BlackScholesLognormalPut_displaced(S, K, r, sigma, T, beta) for K in strikes]
